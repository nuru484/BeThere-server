 ...
 */
const extractFieldNames = (meta) => {
  if (!meta) return [];

  if (hasProperty(meta, "target") && Array.isArray(meta.target)) {
    return meta.target.filter((item) => typeof item === "string");
  }
 ...
 */
const handleRecordNotFoundError = (error) => {
 ...
 */
const handleNullConstraintError = (error) => {
  const fields = extractFieldNames(error.meta);
  const formattedFields = fields.map(formatFieldName).join(", ");
 ...
 */
const handleValueTooLongError = (error) => {
  const fields = extractFieldNames(error.meta);
  const formattedFields = fields.map(formatFieldName).join(", ");
